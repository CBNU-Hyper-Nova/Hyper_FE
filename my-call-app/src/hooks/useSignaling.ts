...
export const useSignaling = (mySignalingId: string, callbacks: SignalingProps = {}) => {
	...
	const { setIsReceiving, setCallerInfo, setIsPending, setIsInCall, setIsCalling, setLocalStream } = useCallStore();
	...
	const createStompClient = useCallback((myId: string) => {
		if (!myId || isConnecting.current) return null;

		const stompClient = new Client({
			brokerURL: 'ws://localhost:8679/ws',
			connectHeaders: {
				login: myId
			},
			debug: function (str) {
		...
		stompClient.onConnect = () => {
			...
			stompClient.subscribe(`/user/${myId}/queue/messages`, (message) => {
				try {
	...
	const signalSend = useCallback((type: string, payload: any) => {
